...
Provider API calls use `globalThis.fetch` with a timeout (30 s; 60 s for Parampos). Pass `fetch` to use your own implementation, for example a proxy-aware fetch or a stub in tests. It can also be set per provider in `config.fetch`.
