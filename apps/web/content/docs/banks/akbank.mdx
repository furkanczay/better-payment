...
<Callout type="info">
  BIN and installment queries are not supported for Akbank: its Sanal POS API has no installment-rate query. `binCheck()` and `installmentInfo()` throw `NOT_SUPPORTED`. Installment plans come from your Akbank agreement: send `installment` in the payment request and compute `paidPrice` from your contracted rates.
</Callout>
