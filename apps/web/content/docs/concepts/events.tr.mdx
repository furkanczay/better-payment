...
Olaylar her sağlayıcı için yayınlanır; çağrı ödeme nesnesinde, bir sağlayıcıda ya da [HTTP handler](/docs/concepts/handler) üzerinden yapılmış olabilir.
...
Sağlayıcılar aynı callback'i birden fazla kez gönderebilir. HTTP handler [callback'leri tekilleştirir](/docs/concepts/handler#duplicate-callbacks-and-idempotency-key); tekrarlanan bir gönderim yeni bir olay yayınlamaz. `completeThreeDSPayment()`'ı kendiniz çağırıyorsanız tekilleştirmeyi dinleyicinizde yapın.
