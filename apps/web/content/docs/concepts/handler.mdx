...
Plugins can add routes to the handler ([plugin endpoints](/docs/guides/writing-plugins#endpoints)). The `authorize` hook runs for them too, with `provider: undefined` and `action: 'plugin:<plugin id>/<endpoint name>'`. For callbacks, [payment events](/docs/concepts/events) are emitted right before `onCallback`, and are retried with it when a listener fails.
