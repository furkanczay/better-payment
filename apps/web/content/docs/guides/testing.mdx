...
A common pattern is to build the `BetterPayment` config from the environment and pass `mock` in tests. Handler routes are `/api/pay/mock/...`, and the browser client has `client.mock`.
