...
`defaultProvider` yoksa (ve birden fazla sağlayıcı varsa) bir hook seçene kadar `ctx.provider` `undefined`'dır. [Ödeme olaylarındaki](/docs/concepts/events) `event.provider` değerini siparişle birlikte saklayın: iade ve durum sorguları ödemeyi alan sağlayıcıya gitmelidir.
...
`endpoints`, [HTTP handler'a](/docs/concepts/handler) adres ekler. Handler, endpoint'in döndürdüğünü `200` durum koduyla JSON olarak ya da `ctx.json(body, status)` ile oluşturulan yanıtı döner:
