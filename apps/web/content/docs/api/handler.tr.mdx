...
Plugin'ler handler'a yeni adresler ekleyebilir ([plugin endpoint'leri](/docs/plugins/writing-plugins#endpoints)). `authorize` hook'u bunlar için de çalışır; `provider: undefined` ve `action: 'plugin:<plugin id>/<endpoint adı>'` olur. Callback'lerde [ödeme olayları](/docs/plugins/events) `onCallback`'ten hemen önce yayınlanır; bir dinleyici hata verirse `onCallback` ile birlikte yeniden denenir.
