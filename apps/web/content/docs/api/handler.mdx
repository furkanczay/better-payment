...
Mutating routes (`payment`, `payment/init-3ds`, `refund`, `cancel`, iyzico checkout/PWI/subscription management) also honor an **`Idempotency-Key`** header. Send a unique key per operation, for example a UUID or `refund-<orderId>`:
