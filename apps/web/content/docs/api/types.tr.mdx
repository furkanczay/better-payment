---
title: Tip referansı
description: better-payment'in dışa açtığı TypeScript tipleri.
---
...
## Sağlayıcı config'leri [#provider-configs]
...
Tüm sağlayıcı config'leri ayrıca `baseUrl`, `locale`, `logger` ve `retry` kabul eder.
...
Ödeme işlemleri sağlayıcı reddettiğinde hata fırlatmaz; `status: 'failure'` ile döner. `binCheck()`, sorgu başarısız olduğunda ya da desteklenmediğinde hata fırlatır.
