...
Providers write error messages in their own words and language, often in technical terms ("Do not honour", "Invalid merchant"). The `localizedErrors` plugin replaces the `errorMessage` of failed results with a message for the customer, chosen by the normalized [error code](/docs/reference/error-codes). English, Turkish, German, and Russian messages are included.
