...
Events are emitted for every provider, and for calls made on the payment object, on a provider, and through the [HTTP handler](/docs/api/handler).
...
Providers can send the same callback more than once. The HTTP handler [deduplicates callbacks](/docs/api/handler#duplicate-callbacks-and-idempotency-key), so a repeated delivery emits no new event. When you call `completeThreeDSPayment()` yourself, deduplicate in your listener.
