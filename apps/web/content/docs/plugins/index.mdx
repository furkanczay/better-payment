...
| ---- | ------------ | ------- |
| `events` | Listens to [payment events](/docs/plugins/events) | Update orders, send e-mails, keep an audit log |
...
Official plugins will ship in the package, under `better-payment/plugins`. Planned:

- Localized error messages, in English and Turkish ([#76](https://github.com/furkanczay/better-payment/issues/76))
- Commission router: send each payment to the cheapest provider ([#51](https://github.com/furkanczay/better-payment/issues/51))

Until then, [write your own](/docs/plugins/writing-plugins). It takes a few lines.
