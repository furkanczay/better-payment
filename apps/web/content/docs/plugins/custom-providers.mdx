...
A provider is a class that extends `PaymentProvider`. The built-in providers are written the same way, so a custom provider works with everything else: plugins and [events](/docs/plugins/events), the [HTTP handler](/docs/api/handler) (at `/api/pay/<id>/...`), the browser client (`client.use('<id>')`) and request validation.
...
| `errorCodeTable()` and `withErrorCode(result)` | Map the provider's error codes to the [normalized codes](/docs/api/error-codes); `withErrorCode` adds `code` to failed results |
