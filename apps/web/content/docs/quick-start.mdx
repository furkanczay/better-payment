...
`failure`. The [HTTP handler](/docs/api/handler) does this for you.
...
Instead of checking every result, listen to [payment events](/docs/plugins/events). They are emitted for every provider and flow, after the provider verified the result:
